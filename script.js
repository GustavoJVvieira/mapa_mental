// ====================================================================
// script.js (Versão Final com Chamada Direta no Frontend)
// ====================================================================
...
// CONFIGURAÇÃO
// ===============================
// ⚠️ ATENÇÃO: A CHAVE FICA EXPOSTA NO FRONTEND. USE COM CAUTELA.
const GEMINI_API_KEY ="AIzaSyBvqBe5dKexHUXJcHdqAHaYimKBuEN1nKc"; 
const genAI = new GoogleGenerativeAI(GEMINI_API_KEY);
// Usando o modelo 'gemini-1.5-flash' conforme solicitado. Se o erro 404 persistir, troque para 'gemini-2.5-flash'.
const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" }); 

// ===============================
// LÓGICA DO BOTÃO "GERAR COM GEMINI" (Chamada Direta)
// ===============================
...
      throw new Error("O formato retornado não parece válido. Resposta: " + response);
    }

    document.getElementById("input").value = response;
    
    // Desenha o mapa automaticamente após a geração bem-sucedida
    const docs = parseInput(response);
    if(docs.length > 0) {
        draw(docs[0]);
        alert("✅ Mapa mental gerado e desenhado!");
    } else {
        alert("✅ Mapa mental gerado! Clique em 'Parser & Desenhar' se o desenho automático falhar.");
    }
...
// REMOVIDO: A função generateMapFromGemini e a lógica de backend foram removidas.
...
  
  // REMOVIDO: Linha original que ligava o botão à função de backend.
